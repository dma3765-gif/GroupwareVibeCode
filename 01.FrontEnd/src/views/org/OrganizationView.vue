  ...
  <div>
    ...
    <el-row :gutter="16">
        ...
        <el-card>
          <template #header>
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <b>조직도</b>
              <el-button v-if="isAdmin" type="primary" size="small"
                @click="showOrgDialog = true; editOrg = null; orgForm = { name:'', code:'', description:'', parentId:'' }">
                + 추가
              </el-button>
            </div>
          </template>
          ...
          >
            <template #default="{ node, data }">
              ...
              <span style="margin-left: 4px; color: #999; font-size: 11px;">
      ...
      <el-col :span="16">
        ...
        <el-card v-else>
          <el-empty description="조직도를 선택하세요" />
        </el-card>
      </el-col>
    </el-row>

    <el-dialog v-model="showOrgDialog" title="조직 정보" width="450px">
      <el-form :model="orgForm" label-width="80px">
        ...
        <el-form-item label="상위조직">
          <el-select v-model="orgForm.parentId" clearable style="width: 100%;">
            <el-option v-for="o in flatOrgsComputed" :key="o.id" :label="o.name" :value="o.id" />
          </el-select>
        </el-form-item>
...
const selectedOrg = ref<Record<string,any>|null>(null)
const loadingMembers = ref(false)
const showOrgDialog = ref(false)
const editOrg = ref<Record<string,any>|null>(null)
const saving = ref(false)
const orgForm = ref({ name: '', code: '', description: '', parentId: '' })

async function fetchTree() {
  try {
    const r = await apiClient.get('/organizations/tree')
    treeData.value = r.data.data ? [r.data.data] : []
  } catch { /* ignore */ }
}

function flatten(nodes: Record<string,any>[], arr: Record<string,any>[] = []): Record<string,any>[] {
  for (const n of nodes) {
    arr.push(n)
    if ((n.children as Record<string,any>[])?.length) flatten(n.children as Record<string,any>[], arr)
  }
  return arr
}
...
async function onOrgClick(data: Record<string,any>) {
  selectedOrg.value = data
  loadingMembers.value = true
  try {
    const r = await apiClient.get('/users', { params: { departmentId: data.id, page: 1, pageSize: 100 } })
    members.value = r.data.data?.items || []
  } catch {
    members.value = []
  } finally {
...
function editOrgOpen() {
  editOrg.value = selectedOrg.value
  if (selectedOrg.value) {
    orgForm.value = {
      name: selectedOrg.value.name as string,
      code: selectedOrg.value.code as string,
      description: (selectedOrg.value.description as string) || '',
      parentId: (selectedOrg.value.parentId as string) || '',
    }
  }
...
async function saveOrg() {
  ...
  try {
    if (editOrg.value) {
      await apiClient.put(`/organizations/${editOrg.value.id}`, orgForm.value)
    } else {
      await apiClient.post('/organizations', orgForm.value)
    }
...
async function deleteOrg() {
  ...
  try {
    await apiClient.delete(`/organizations/${selectedOrg.value!.id}`)
